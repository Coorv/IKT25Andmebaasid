...
update Employees
set LastName = 'Bond', MiddleName = '007'
where Id = 9
...
--tuleb vea teade kuna kutsusime välja int-i, aga Tom on nvarchar
...
--see konverteerb ASCII tähe väärtuse numbriks
select ASCII('A')
...
--paremal poolt tühijad stringid lõikab ära
select RTRIM('      Hello        ')

--keerab kooloni sees olevad andmed vastupidiseks
--vastavalt siis lower-ga ja upper-ga saan muuta märkide suurust
--reverse funktsioom pöörab kõik ümber
...
--kuvab siis @-tähemärgi asetust e mitmes on @-märk
...
--alates siis teisest tähes emailis kuni @ märgini on tärnid
...
select Email, PATINDEX('%@aaa.com', Email) as FirstOccurence from Employees
where PATINDEX('%@aaa.com', Email) > 0
--leian kõik selle domeeni esindajad ja alates mitmendast märgist algab @

--kõik .com emaili asendab .net-ga
select Email, replace(Email, '.com', '.net') as ConvertedEmail
from Employees

--soovin asendad peale esimest märki kolm tähte viie tärniga
...
where c_datetimeoffset = '2026-03-19 14:29:45.0600000 +00:00'

select CURRENT_TIMESTAMP, 'CURRENT_TIMESTAMP' --aja päring
select SYSDATETIME(), 'SYSDATETIME' --veel täpsem
...
select Name, DateOfBirth, DateName(WEEKDAY, dateOfBirth) as [Day],
	--vaatab DoB veerust kuupäeva ja kuvad kuu nr
...
select Dateadd(DAY, -20 , '2026-03-24 12:59:30.670')
--kuvab kahe stringis oleva kuudevahelist aega nr-ga
...
on SalesLT.ProductModel.ProductModelID = SalesLT.ProductModelProductDescription.ProductModelID

select SalesLT.ProductCategory.ProductCategoryID, SalesLT.ProductCategory.rowguid, SalesLT.ProductCategory.ModifiedDate 
from SalesLT.ProductCategory
right join SalesLT.Product
on SalesLT.ProductCategory.ProductCategoryID = SalesLT.Product.ProductCategoryID

select SalesLT.CustomerAddress.AddressID, SalesLT.CustomerAddress.rowguid, SalesLT.CustomerAddress.ModifiedDate 
from SalesLT.CustomerAddress
inner join SalesLT.Address
on SalesLT.CustomerAddress.AddressID = SalesLT.Address.AddressID

select SalesLT.ProductDescription.ProductDescriptionID, SalesLT.ProductDescription.rowguid, SalesLT.ProductDescription.ModifiedDate 
from SalesLT.ProductDescription
full outer join SalesLT.ProductModelProductDescription
on SalesLT.ProductDescription.ProductDescriptionID = SalesLT.ProductModelProductDescription.ProductModelID

select SalesLT.SalesOrderHeader.SalesOrderID, SalesLT.SalesOrderHeader.rowguid, SalesLT.SalesOrderHeader.ModifiedDate 
from SalesLT.SalesOrderHeader
cross join SalesLT.SalesOrderDetail	
where SalesLT.SalesOrderHeader.SalesOrderID = SalesLT.SalesOrderDetail.SalesOrderID


create proc spGetAllCustomers
as begin
	select CustomerId, Title, FirstName from SalesLT.Customer
end
...
execute spGetOrdersByDateRange '2008-06-13 00:00:00.000', '2008-06-13 00:00:00.000'
...
@SellStartDate date
as begin
	insert into SalesLT.Product values (@Name, @ProductNumber, @StandardCost, @ListPrice, GETDATE())
end
...
as begin
	delete from SalesLT.Customer where CustomerID not in (select CustomerID from SalesLT.SalesOrderHeader)
end
...
--ei muuda andmeid multi state funktsioonis,
--kuna see on nagu stored procedure

--deterministic vs non-deterministic funktsioonid
--deterministic funktsioonid annavad alati sama tulemuse, kui sisend on sama
select count (*) from EmployeeWithDates
select SQUARE(4)

--non-deterministic funktsioonid annavad erineva tulevuse, kui sisend on sama
select GETDATE()
...
returns table as
return (select Name from SalesLT.Product where ListPrice between @MinPrice and @MaxPrice)

select * from fn_GetProductsByPrice_ITVF(1, 2000)
...
alter function fn_MS_GetCustomerFullInfo_MSTVF(@CustomerID int)
returns @Result table
		...
		as begin
		insert into @Result
		select coalesce(FirstName, LastName) as Name, EmailAddress, Phone from SalesLT.Customer
		where @CustomerID = CustomerID
		return 
		end

select * from fn_MS_GetCustomerFullInfo_MSTVF(1)

create function fn_MS_GetCustomerOrderSummary_MSTVF(@CustomerID)
returns @Result Table (TotalDue int,
...
--indekseeritud view
--MS SQL-s on indekseeritud view nime alla ja
--Oracle's materjaliseeritud view nimega
...
--antud juhul kasutame ISNULL funktsiooni asendamaks NULL väärtust
--3. kui groupby on täpsustatud, siis view select list peab 
--sisaldama Count(*) väljendit
--4. Baastabelis peaksid view'd olema viidatud kaheosaliese nimega
...
insert into ##TestTempTable values(103, 'Pam', 'Male')
...
--view-id ja funktsiooe ei saa teha ajutistele tabelitele

--Triggerid

--DML trigger
--kokku on kolme tüüpi: DML, DD ja LOGON
...
--after trigger käivitub peale sündmust, kus kuskil on tehtud insertm
--update ja delete
...
as begin
	...
	begin
		set @AuditString = ''
		--selekteerib esimese rea andmed temo tabel-st
